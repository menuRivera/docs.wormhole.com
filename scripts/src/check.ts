...
(async function () {
  ...
  for (const chain of chains) {
    ...
    for (const network of ["Mainnet", "Testnet", "Devnet"]) {
      const sdkChain = newconf[network as Network].chains[
        sdkName
      ] as SdkChainConfig<Network, Chain>;

      if (sdkChain === undefined) {
        console.log("UNDEFINED FOR ", chain.name);
        continue;
      }
      ...
      if (!nativeChainId || nativeChainId.id === "N/A") {
        console.log("Missing chain id for: " + sdkName + " " + network);
        continue;
      }

      if (nativeChainId.id !== sdkChain.nativeChainId.toString())
        console.log(
